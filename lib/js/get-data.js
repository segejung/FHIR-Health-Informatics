    ...
    client.request("Observation?" + query, {
        ...
        function (ob) {
            ...
            function displaySysAnalysis() {
                if (p.sys < 120) {
                    sys_analysis.innerHTML = "Normal";
                } else if (p.sys > 120 && p.sys < 129) {
                    sys_analysis.innerHTML = "Elevated";
                } else if (p.sys > 130 && p.sys < 139) {
                    sys_analysis.innerHTML = "High Blood Pressure Stage 1";
                } else if (p.sys > 140 && p.sys < 179) {
                    sys_analysis.innerHTML = "High Blood Pressure Stage 2";
                } else if (p.sys > 180) {
                    sys_analysis.innerHTML = "Hypertensive Crisis";
                }
            }
            displaySysAnalysis();
            ...
            function displayDiaAnalysis() {
                if (p.dia < 80) {
                    bp_analysis.innerHTML = "Normal";
                } else if (p.dia > 80 && p.dia < 89) {
                    bp_analysis.innerHTML = "High Blood Pressure Stage 1";
                } else if (p.dia > 90 && p.dia < 119) {
                    bp_analysis.innerHTML = "High Blood Pressure Stage 2";
                } else if (p.dia > 120) {
                    bp_analysis.innerHTML = "Hypertensive Crisis";
                }
            }
            displayDiaAnalysis();

            function displayNagler() {
                let myJSON = parseFloat(obs.nagler);
                //nagler_formula.innerHTML = "Recommended weight for your height is " + p.nagler;
                document.getElementById("nagler_formula").innerHTML = "Recommended weight for your height is Nagler's formula" + displayObservation(p.nagler);
                //nagler_formula2.innerHTML = "You are " + (weight[0].valueQuantity.value - p.nagler) + " over.";
...
//let bmi = (weight[0].valueQuantity.value)/(0.01*height[0].valueQuantity.value)^2;
